...
const hoteis = [
    { nome: "Pet Hotel Alpha", endereco: "Rua das Flores, 123, São Paulo", lat: -23.505, lng: -46.850, imagens: ["default1.jpg", "default2.jpg", "default3.jpg"], rating: 5 },
    { nome: "Pet House Barueri", endereco: "Av. Barueri, 456, Barueri", lat: -23.511, lng: -46.875, imagens: ["default1.jpg", "default2.jpg", "default3.jpg"], rating: 5 },
    { nome: "Dog Resort Jandira", endereco: "Rua Jandira, 789, Jandira", lat: -23.532, lng: -46.902, imagens: ["default1.jpg", "default2.jpg", "default3.jpg"], rating: 5 },
];
...
function adicionarMarcadorUsuario(center) {
    const userIcon = L.divIcon({ className: "user-marker", html: `<div style="width:26px;height:26px;background:#2563EB;border:3px solid white;border-radius:50%;box-shadow:0 4px 8px rgba(0,0,0,0.3),0 0 12px rgba(37,99,235,0.5);"></div>`, iconSize: [26,26], iconAnchor: [13,13] });
...
function adicionarMarcadorHotel(hotel) {
    const icone = L.divIcon({ className: "hotel-marker", html: `<div style="width:24px;height:24px;background:linear-gradient(145deg,#ff4b4b,#e70000);border:3px solid white;border-radius:50%;box-shadow:0 3px 6px rgba(0,0,0,0.3);"></div>`, iconSize: [24,24], iconAnchor: [12,12] });
    L.marker([hotel.lat, hotel.lng], { icon: icone }).addTo(map).on("click", () => exibirHotel(hotel));
}
...
async function obterCoordenadas(endereco) {
    try {
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(endereco)}`, { headers: { "User-Agent": "PetMap/1.0" } });
...
async function cadastrarHotel(nome,endereco) {
    ...
    if (!coords) return;
    const novoHotel = { nome, endereco, lat: coords.lat, lng: coords.lng, imagens: ["default1.jpg","default2.jpg","default3.jpg"], rating: 5 };
